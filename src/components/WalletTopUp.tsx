...
const WalletTopUp: React.FC = () => {
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-8 glass rounded-2xl">
      <h2 className="text-2xl font-bold mb-4">{t('auto.auto_wallet_top_up_2994')}</h2>
      {!showCheckout ? (
        <>
          <div className="mb-4">
            <label className="block text-sm font-medium mb-2">{t('auto.auto_amount__usd__2993')}</label>
            <input
              type="number"
              value={amount}
          ...
          <button onClick={() => setShowCheckout(true)} className="w-full bg-primary text-white py-3 rounded-lg font-bold">{t('auto.auto_proceed_to_payment_2992')}</button>
        </>
      ) : (
