...
import { generateFashionSuggestions, synthesizeWeather } from '../services/gemini';
...
export default function DigitalTailor() {
  ...
  const handleAction = async (item: FashionItem, type: 'buy' | 'rent') => {
    if (!auth.currentUser) return;
    try {
      await addDoc(collection(db, 'rentals'), {
        ...
        price: type === 'buy' ? item.price : item.price * 0.2,
        type,
        status: type === 'buy' ? 'purchased' : 'active',
        destination: selectedItinerary?.destination || destination,
        createdAt: new Date().toISOString(),
        rentalEndDate: type === 'rent' ? new Date(Date.now() + 7 * 24 * 3600 * 1000).toISOString() : null
      });
      // Add transaction
      await addDoc(collection(db, 'transactions'), {
        userId: auth.currentUser.uid,
        amount: type === 'buy' ? item.price : item.price * 0.2,
        type: 'debit',
        description: `${type === 'buy' ? 'Purchased' : 'Rented'} ${item.name}`,
        category: 'booking',
      ...
      <div className="flex flex-col md:flex-row gap-12">
        ...
        <div className="w-full md:w-1/3 space-y-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-4xl font-display font-bold mb-2">Digital Tailor</h1>
              <p className="text-foreground/50">Synthesize your travel style with Ruth AI.</p>
            </div>
            <button 
