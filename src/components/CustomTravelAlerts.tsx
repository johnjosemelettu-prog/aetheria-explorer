...
export default function CustomTravelAlerts() {
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-50 px-6 py-20 lg:p-24 selection:bg-rose-500/30">
      <div className="max-w-7xl mx-auto">
        <motion.div 
          ...
          className="mb-16"
        >
          <h1 className="text-5xl md:text-7xl font-display font-medium tracking-tight mb-6">{t('auto.auto_customizable_travel__928')}</h1>
          <div className="w-24 h-1 bg-gradient-to-r from-rose-500 to-orange-500 mb-8" />
          <p className="text-xl text-neutral-400 max-w-2xl leading-relaxed">
            {t('auto.auto_discover_a_curated_c_927')}
                                </p>
        </motion.div>
          ...
          {items.map((i, idx) => (
            ...
            >
              <img 
                ...
                alt={t('auto.auto_feature_item_926')}
              ...
              <div className="absolute bottom-0 left-0 p-8">
                <p className="text-rose-400 font-mono text-xs tracking-widest uppercase mb-2">{t('auto.auto_item_0_925')}{i}</p>
                <h3 className="text-2xl font-bold text-white mb-2">{t('auto.auto_discovery___924')}{i}</h3>
                <p className="text-neutral-300 text-sm line-clamp-2">{t('auto.auto_incredible_localized_923')}</p>
              </div>
            </motion.div>
          ))}
        </div>
