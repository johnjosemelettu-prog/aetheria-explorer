...
export default function ComedyClubNights() {
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-50 px-6 py-20 lg:p-24 selection:bg-pink-500/30">
      <div className="max-w-7xl mx-auto">
        <motion.div 
          ...
          className="mb-16"
        >
          <h1 className="text-5xl md:text-7xl font-display font-medium tracking-tight mb-6">{t('auto.auto_comedy_club_nights_833')}</h1>
          <div className="w-24 h-1 bg-gradient-to-r from-pink-500 to-rose-500 mb-8" />
          <p className="text-xl text-zinc-400 max-w-2xl leading-relaxed">
            {t('auto.auto_explore_our_curated__832')}
                                </p>
        </motion.div>
          ...
          {items.map((i, idx) => (
            ...
            >
              <img 
                ...
                alt={t('auto.auto_feature_item_831')}
              ...
              <div className="absolute bottom-0 left-0 p-8">
                <p className="text-pink-400 font-mono text-xs tracking-widest uppercase mb-2">{t('auto.auto_utility_item_830')}</p>
                <h3 className="text-2xl font-bold text-white mb-2">{t('auto.auto_enhancement___829')}{i}</h3>
                <p className="text-zinc-300 text-sm line-clamp-2">{t('auto.auto_a_specialized_soluti_828')}</p>
              </div>
            </motion.div>
          ))}
        </div>
