      ...
      <section className="glass p-8 rounded-3xl">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-display font-bold">{t('auto.auto_global_esim_1377')}</h2>
          </div>
          <button 
        ...
        <div className="space-y-4">
          {esims.length > 0 ? (
                ...
                <div className="relative z-10">
                  <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-2">
                      <span className="text-lg font-bold">{esim.country} {t('auto.auto_premium_1376')}</span>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between text-[10px] text-foreground/40 uppercase tracking-widest font-bold">
                      <span className="flex items-center gap-1">
                        ...
                        {esim.dataUsed.toFixed(1)} {t('auto.auto_gb_used_1375')}
                                                          </span>
                      <span className="flex items-center gap-1">
                        <BarChart3 className="w-3 h-3" />
                        {esim.dataLimit} {t('auto.auto_gb_total_1374')}
                                                          </span>
                    </div>
                  </div>
                </div>
            ...
            <div className="p-12 rounded-3xl text-center border-dashed border-2 border-white/5">
              ...
              <p className="text-sm text-foreground/40">{t('auto.auto_no_active_esim_profi_1373')}</p>
            </div>
          )}
        </div>
        ...
        >
          ...
          {t('auto.auto_synthesize_new_esim_1372')}
                          </button>
      </section>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-2xl font-display font-bold">{t('auto.auto_esim_synthesis_1371')}</h2>
              ...
              <div className="space-y-6">
                <div>
                  <label className="text-sm font-bold text-foreground/50 uppercase tracking-widest mb-2 block">{t('auto.auto_select_country_1370')}</label>
                  <select
                  ...
                  >
                    <option value="" disabled className="bg-background">{t('auto.auto_choose_a_destination_1369')}</option>
                    {countries.map(c => (
                ...
                <div className="p-4 rounded-2xl bg-secondary/10 border border-secondary/20">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-sm font-bold">{t('auto.auto_premium_synthesis_1368')}</span>
                  </div>
                  <p className="text-xs text-foreground/60 leading-relaxed">
                    {t('auto.auto_instant_activation___1367')}
                                                        </p>
                </div>
