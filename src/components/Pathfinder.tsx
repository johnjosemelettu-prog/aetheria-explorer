    ...
    <div className="relative min-h-screen bg-neutral-900 text-white overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 p-6 flex justify-between items-start pointer-events-none">
        <div className="glass p-4 rounded-2xl pointer-events-auto">
          <h1 className="text-xl font-bold flex items-center gap-2"><Compass className="text-emerald-400" /> {t('auto.auto_pathfinder_1979')}</h1>
          <p className="text-xs text-white/50 uppercase tracking-widest mt-1">{t('auto.auto_ar_navigation_active_1978')}</p>
        </div>
      ...
      <div className="relative z-10 mt-auto p-6 pointer-events-none">
        <div className="glass p-6 rounded-3xl max-w-md mx-auto pointer-events-auto border border-emerald-500/30 shadow-[0_0_50px_rgba(52,211,153,0.1)]">
          <div className="flex justify-between items-center mb-6">
             <div>
               <h2 className="text-3xl font-display font-bold">{t('auto.auto_1_2_km_1977')}</h2>
               <p className="text-emerald-400 font-mono">{t('auto.auto_15_mins_walking_1976')}</p>
             </div>
          ...
          <div className="space-y-4">
            <div className="p-4 bg-black/40 rounded-xl border border-white/5">
              <span className="text-xs text-white/50 font-bold block mb-1">{t('auto.auto_next_turn_1975')}</span>
              <p className="font-bold text-lg">{t('auto.auto_turn_right_at_neon_a_1974')}</p>
            </div>
            <button className="w-full py-4 bg-white/10 hover:bg-white/20 transition-colors rounded-xl font-bold">
              {t('auto.auto_end_navigation_1973')}
                                      </button>
          </div>
        </div>
      </div>
