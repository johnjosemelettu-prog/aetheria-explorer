    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-accent/20 via-background to-primary/10 flex-col justify-between p-12 border-r border-white/5">
        <div className="flex items-center gap-3">
          <img src="/logo.png" alt="Aetheria" className="w-9 h-9" onError={e => (e.currentTarget.style.display='none')} />
        ...
        <div>
          ...
          <h2 className="text-4xl font-display font-bold mb-4 leading-tight">
            Support Agent<br />Command Centre
          </h2>
          <p className="text-foreground/50 mb-10 text-lg leading-relaxed">
            Resolve traveller issues, manage support tickets, and deliver exceptional customer experiences.
          </p>
        ...
        <p className="text-foreground/20 text-xs">© 2026 Aetheria Explorer. Agent Portal v2.0</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-md">
          <div className="flex items-center gap-2 mb-8 lg:hidden">
            ...
            <span className="text-lg font-display font-bold tracking-tighter">Support Agent Portal</span>
          </div>
          ...
          <AnimatePresence mode="wait">
            {tab === 'signin' && (
              <motion.form key="signin"
                ...
                <div>
                  <label className="text-xs font-semibold text-foreground/40 uppercase tracking-widest mb-1 block">Agent Email</label>
                  <div className="relative">
                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground/30" />
                    <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                      placeholder="agent@aetheria-support.com" className={inputClass} />
                  </div>
                </div>
                <div>
                  <label className="text-xs font-semibold text-foreground/40 uppercase tracking-widest mb-1 block">Password</label>
                  <div className="relative">
                ...
                <p className="text-center text-xs text-foreground/30 pt-1">
                  Forgot password?{' '}
                  <button type="button" onClick={() => setTab('forgot')} className="text-accent hover:underline font-semibold">Reset it</button>
                </p>
              </motion.form>
            )}
              ...
              <motion.div key="forgot"
                ...
                {forgotSent ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-foreground/50 text-sm mb-6">Check your inbox at <strong className="text-white">{forgotEmail}</strong>.</p>
                    <button onClick={() => { setTab('signin'); setForgotSent(false); }}
                      className="text-accent hover:underline text-sm font-semibold">Back to Sign In</button>
                  </div>
                ) : (
                  <form onSubmit={handleForgot} className="space-y-4">
                    <div className="text-center mb-6">
                      ...
                      <p className="text-foreground/50 text-sm">Enter your agent email and we'll send a reset link.</p>
                    </div>
                    <div className="relative">
                      <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground/30" />
                    ...
                    <p className="text-center text-xs">
                      <button type="button" onClick={() => setTab('signin')} className="text-accent hover:underline font-semibold">Back to Sign In</button>
                    </p>
                  </form>
                )}
              </motion.div>
