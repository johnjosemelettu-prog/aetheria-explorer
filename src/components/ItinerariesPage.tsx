      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="glass p-6 rounded-3xl">
            <h3 className="text-sm font-bold text-foreground/50 uppercase tracking-widest mb-4">{t('auto.auto_search_1552')}</h3>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground/30" />
              <input 
                type="text" 
                placeholder={t('auto.auto_destination____1551')}
                value={searchQuery}
          ...
          <div className="glass p-6 rounded-3xl">
            <h3 className="text-sm font-bold text-foreground/50 uppercase tracking-widest mb-4">{t('auto.auto_filter_by_1550')}</h3>
            <div className="space-y-3">
              <button className="w-full flex items-center justify-between p-3 rounded-xl glass-hover text-sm font-bold">
                <span className="flex items-center gap-2"><Calendar className="w-4 h-4" /> {t('auto.auto_duration_1549')}</span>
                <ChevronRight className="w-4 h-4 text-foreground/20" />
              </button>
              <button className="w-full flex items-center justify-between p-3 rounded-xl glass-hover text-sm font-bold">
                <span className="flex items-center gap-2"><Leaf className="w-4 h-4" /> {t('auto.auto_carbon_footprint_1548')}</span>
                <ChevronRight className="w-4 h-4 text-foreground/20" />
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {loading ? (
              <div className="py-20 text-center">
                ...
                <p className="text-sm text-foreground/40">{t('auto.auto_loading_synthesis____1547')}</p>
              </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-sm text-foreground/50">
                          ...
                          <span className="flex items-center gap-1.5 px-3 py-1 bg-green-500/10 text-green-400 rounded-full">
                            ...
                            {itinerary.carbonFootprint}{t('auto.auto_kg_co2_offset_1546')}
                                                                    </span>
                          <span className="flex items-center gap-1.5 px-3 py-1 bg-white/5 rounded-full">
                            <Clock className="w-3.5 h-3.5" />
                            {itinerary.activities.length} {t('auto.auto_activities_1545')}
                                                                    </span>
                          {itinerary.invitedUsers && itinerary.invitedUsers.length > 0 && (
                            <span className="flex items-center gap-1.5 px-3 py-1 bg-white/5 rounded-full text-primary">
                              ...
                              {itinerary.invitedUsers.length} {t('auto.auto_invited_1544')}
                                                                          </span>
                          )}
                        </div>
                      </div>
