       ...
       <div className="max-w-5xl mx-auto relative z-10">
          <header className="mb-12">
            <h1 className="text-5xl font-display font-bold flex items-center gap-4 text-indigo-300">
               ...
               {t('auto.auto_global_treasure_hunt_1409')}
                                  </h1>
            <p className="text-indigo-200/70 mt-2 font-mono ml-14">{t('auto.auto_aetheria_artifacts_a_1408')}</p>
          </header>
          ...
          <div className="grid grid-cols-3 gap-8">
             <div className="col-span-2 bg-indigo-900/30 border border-indigo-400/30 rounded-3xl p-6 relative min-h-[500px]">
                ...
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center">
                   ...
                   <h3 className="font-display text-2xl tracking-widest text-indigo-200">{t('auto.auto_artifact__008_1407')}</h3>
                   <div className="bg-black/40 mt-4 p-4 rounded-xl border border-indigo-500/20 max-w-sm mx-auto backdrop-blur-md">
                      <p className="font-mono text-sm text-cyan-300">
                        {t('auto.auto__where_the_metal_spi_1406')}
                                                        </p>
                   </div>
                </div>
             </div>
             ...
             <div className="flex flex-col gap-6">
                <div className="bg-indigo-900/30 border border-indigo-400/30 rounded-3xl p-6 flex-1">
                   <h3 className="text-indigo-300 font-bold mb-4 flex items-center gap-2">
                     <Map className="w-5 h-5" /> {t('auto.auto_recently_found_1405')}
                                                 </h3>
                   <ul className="flex flex-col gap-4 font-mono text-xs">
                     <li className="flex justify-between border-b border-indigo-500/20 pb-2">
                        <span className="text-cyan-400">{t('auto.auto__007__the_jade_coin__1404')}</span>
                        <span className="text-indigo-200">{t('auto.auto_kyoto_1403')}</span>
                     </li>
                     <li className="flex justify-between border-b border-indigo-500/20 pb-2">
                        <span className="text-cyan-400">{t('auto.auto__006__neon_shard__1402')}</span>
                        <span className="text-indigo-200">{t('auto.auto_tokyo_1401')}</span>
                     </li>
                     <li className="flex justify-between items-center text-indigo-400/50 pt-2">
                        {t('auto.auto_view_all_archives____1400')}
                                                       </li>
                   </ul>
                </div>
                
                <div className="bg-cyan-900/20 border border-cyan-400/30 rounded-3xl p-6">
                   <div className="text-cyan-400 font-bold font-mono text-xs mb-1">{t('auto.auto_reward_pool_1399')}</div>
                   <div className="text-3xl font-black text-white font-display mb-4">{t('auto.auto_50_000_exp_1398')}</div>
                   <button className="w-full bg-cyan-500 text-indigo-950 font-bold py-3 rounded-xl hover:bg-cyan-400 transition">
                      {t('auto.auto_enter_the_hunt_1397')}
                                                 </button>
                </div>
             </div>
          </div>
