  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold">Service Listings</h2>
        <button className="flex items-center gap-2 px-4 py-2 bg-primary text-white text-sm font-semibold rounded-xl"><Plus className="w-4 h-4"/>Add Service</button>
      </div>
        ...
        {listings.map(l => (
          <div key={l.id} className="glass rounded-xl p-4 flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-foreground/40">{l.category} · {l.price} · {l.bookings} bookings</p>
            </div>
