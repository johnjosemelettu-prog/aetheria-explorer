    ...
    <div className="min-h-screen bg-black text-white p-4 sm:p-8">
      <div className="max-w-6xl mx-auto">
        <header className="mb-8 flex justify-between items-center">
           <div>
             <h1 className="text-3xl sm:text-5xl font-display font-bold">{t('auto.auto_the_daily_lens_2035')}</h1>
             <p className="text-gray-400 font-mono mt-2 text-xs sm:text-sm">{t('auto.auto_theme__reflections_i_2034')}</p>
           </div>
           <button className="bg-white text-black font-bold py-2 px-4 sm:px-6 rounded-full hover:bg-gray-200 flex items-center gap-2 text-sm">
             <Camera className="w-4 h-4" /> {t('auto.auto_submit_entry_2033')}
                                 </button>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="absolute bottom-8 left-8 right-8 z-20 flex justify-between items-end">
                 <div>
                   <div className="bg-yellow-500 text-black font-bold text-xs px-2 py-1 rounded w-max mb-3 uppercase tracking-widest">
                     {t('auto.auto_yesterday_s_winner_2031')}
                                                     </div>
                   <h2 className="text-4xl font-display font-bold mb-2">{t('auto.auto_shinjuku_puddles_2030')}</h2>
                   <p className="text-gray-300 font-mono">{t('auto.auto_by__tokyonomad___sho_2029')}</p>
                 </div>
           ...
           <div className="bg-[#111] rounded-3xl p-6 border border-white/10 flex flex-col h-[60vh]">
              <h3 className="font-bold text-xl mb-6">{t('auto.auto_today_s_top_entries_2028')}</h3>
                 ...
                 {[...Array(5)].map((_, i) => (
                   ...
                   >
                     <img 
                       ...
                       alt={t('auto.auto_thumbnail_2027')} 
                     ...
                     <div className="flex-1">
                        <h4 className="font-bold text-sm">{t('auto.auto_neon_nights_2026')}</h4>
                        <p className="text-xs text-gray-500 font-mono">{t('auto.auto__user__2025')}{100 + i}</p>
                     </div>
