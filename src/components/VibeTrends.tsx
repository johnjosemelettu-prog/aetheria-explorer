  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-background/80 backdrop-blur-sm p-4 rounded-xl border border-white/10 shadow-lg">
        ...
        <p className="text-sm text-cyan-400">Current Vibe Score: {payload[0].value}</p>
        <p className="text-sm text-foreground/50">Last Week: {payload[0].payload.last}</p>
        <p className={`text-xs font-bold mt-2 ${payload[0].value > payload[0].payload.last ? 'text-green-400' : 'text-red-400'}`}>
          Change: {payload[0].value - payload[0].payload.last > 0 ? '+' : ''}{payload[0].value - payload[0].payload.last}
        </p>
      </div>
    );
  }
  ...
  return (
    <section className="glass p-8 rounded-3xl col-span-1">
       <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-display font-bold">Vibe Trends</h2>
