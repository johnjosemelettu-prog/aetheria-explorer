...
export async function synthesizeWelcomeEmail(input: WelcomeEmailInput): Promise<WelcomeEmailOutput> {
  // Guard for client-side/static-export environment
  if (typeof window !== 'undefined') {
    return { 
      subject: "Welcome to Aetheria", 
      body: `Hello ${input.firstName}, your journey starts now. This is a high-fidelity mock generated because the AI synthesis engine is only available in the hosted environment.` 
    };
  }
  ...
  try {
    ...
    return { 
      subject: "Welcome to Aetheria", 
      body: "Your journey starts now. Welcome to the neural grid." 
    };
  }
}
