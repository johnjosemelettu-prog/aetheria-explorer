...
export default function VisionHubPage() {
  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      else data = await generateHistoricalImage({ photoDataUri, targetEra: "100 years ago", language: currentLang });
    ...
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      <div className="text-center mb-12">
        <h1 className="font-headline text-4xl font-black tracking-tight md:text-5xl flex items-center justify-center gap-3">
          ...
          {t('visionHub.title' as any) || 'Vision AI Hub'}
        </h1>
        <p className="mt-4 text-lg text-muted-foreground">
          {t('visionHub.subtitle' as any) || 'Decode your environment with high-fidelity multimodal analysis.'}
        </p>
      </div>
            ...
            <Card className="overflow-hidden border-2 shadow-2xl rounded-[2.5rem] bg-slate-950">
              ...
              <CardFooter className="p-8 bg-slate-900">
                <Button 
                  ...
                  className="w-full h-16 rounded-2xl font-black text-xl shadow-xl shadow-primary/20 active:scale-95 transition-all bg-primary text-white"
                >
...
function PharmacyResultView({ data }: { data: any }) {
  return (
    <div className="space-y-6">
      <Card className="rounded-[2rem] border-none shadow-xl bg-white p-8">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {data.activeIngredients.map((ing: string) => <Badge key={ing} className="bg-emerald-50 text-emerald-700 border-emerald-100 font-bold">{ing}</Badge>)}
        </div>
