    ...
    return (
      <Card className="mx-auto max-w-2xl border-none shadow-2xl rounded-[2.5rem] overflow-hidden animate-in zoom-in duration-500">
        <div className="bg-emerald-600 p-10 text-white text-center relative overflow-hidden">
          <div className="absolute top-0 right-0 p-10 opacity-10"><Bus className="h-40 w-40" /></div>
          <div className="relative z-10">
            ...
            <CardTitle className="text-4xl font-black font-headline tracking-tighter">Ready for Boarding!</CardTitle>
            <CardDescription className="text-white/80 mt-2 text-lg font-medium">Your coach seat is confirmed.</CardDescription>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-8">
          <Card className="border-none shadow-xl rounded-[2.5rem] bg-white overflow-hidden">
            <CardHeader className="bg-emerald-900 text-white p-8">
              ...
              <CardDescription className="text-slate-400">Specify names for all passengers.</CardDescription>
            </CardHeader>
          ...
          <Card className="border-none shadow-xl rounded-[2.5rem] bg-white p-8">
            <h3 className="font-headline font-black text-xl mb-6 text-slate-900 flex items-center gap-2">
              <Wallet className="h-6 w-6 text-emerald-600" /> Payment Node
            </h3>
            <div className="space-y-4">
              <div className="flex justify-between items-center p-4 rounded-2xl bg-slate-50 border border-slate-100">
                <span className="text-xs font-black uppercase text-slate-400">Available Funds</span>
              ...
              <div className="flex justify-between items-center p-4 rounded-2xl bg-emerald-50 border border-emerald-100">
                <span className="text-xs font-black uppercase text-emerald-600">Total Fare</span>
                <span className="font-headline font-black text-xl text-emerald-600">${selectedBus.price.toFixed(2)}</span>
              </div>
            </div>
          </Card>
        </div>
        ...
        <aside className="lg:col-span-5">
          <Card className="border-none shadow-2xl rounded-[3rem] bg-white overflow-hidden sticky top-24">
            <CardHeader className="bg-emerald-50 p-8">
              <Badge className="bg-emerald-600 text-white border-none font-bold uppercase tracking-widest text-[9px] mb-4">Coach Review</Badge>
              <CardTitle className="text-3xl font-black font-headline text-slate-900">{selectedBus.provider}</CardTitle>
              <CardDescription className="font-bold uppercase tracking-widest text-[10px] mt-1">{selectedBus.seatType}</CardDescription>
            </CardHeader>
            <CardContent className="p-8 space-y-6">
              <div className="flex items-center gap-12 py-6 border-y border-slate-100">
                <div className="text-center">
                  <p className="text-2xl font-black font-headline text-slate-900">{selectedBus.departureTime}</p>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">DEP</p>
                </div>
                <ArrowRight className="text-emerald-600 h-6 w-6" />
                <div className="text-center">
                  <p className="text-2xl font-black font-headline text-slate-900">{selectedBus.arrivalTime}</p>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-xs text-slate-500 font-medium leading-relaxed">Securing your seat earns you +250 Explorer Points.</p>
                </div>
              </div>
            </CardContent>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className="grid grid-cols-1 items-end gap-6 md:grid-cols-4">
                ...
                {tripType === 'roundTrip' ? (
                  ...
                  <Button type="submit" className="h-14 rounded-2xl font-black text-lg shadow-xl shadow-emerald-200 bg-emerald-600 text-white" disabled={isLoading}>
                    {isLoading ? <Loader2 className="animate-spin mr-2" /> : <><Search className="mr-2 h-5 w-5" /> Search Coaches</>}
                  </Button>
                )}
              </div>
              {tripType === 'roundTrip' && (
                <Button type="submit" className="w-full h-14 rounded-2xl font-black text-lg shadow-xl shadow-emerald-200 bg-emerald-600 text-white" disabled={isLoading}>
                  {isLoading ? <Loader2 className="animate-spin mr-2" /> : <><Search className="mr-2 h-5 w-5" /> Search Coaches</>}
                </Button>
              )}
            </form>
      ...
      {searchResults && (
        <div className="grid grid-cols-1 gap-6 max-w-4xl mx-auto">
          {searchResults.map((bus) => (
            <Card key={bus.id} className="overflow-hidden border-none shadow-xl rounded-[2rem] bg-white group hover:border-emerald-500 transition-all duration-500">
              <CardContent className="p-8 flex flex-col md:flex-row items-center justify-between gap-8">
                <div className="flex items-center gap-8 flex-1">
                  <div className="bg-emerald-50 p-6 rounded-3xl text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-colors"><Bus className="h-10 w-10" /></div>
                  <div className="space-y-2">
                    <CardTitle className="text-2xl font-black font-headline text-slate-900">{bus.provider}</CardTitle>
                    <div className="flex flex-wrap gap-2">
                      {bus.amenities.map(a => <Badge key={a} variant="outline" className="text-[8px] font-black uppercase tracking-tighter">{a}</Badge>)}
                    </div>
                    <div className="flex items-center gap-6 text-sm font-bold text-slate-400 pt-1">
                      <div className="flex items-center gap-2"><Clock className="h-4 w-4" /> {bus.departureTime} &rarr; {bus.arrivalTime}</div>
                      <div className="flex items-center gap-2 text-emerald-600"><ShieldCheck className="h-4 w-4" /> {bus.seatType}</div>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-8">
                  <div className="text-right"><p className="text-4xl font-black font-headline text-slate-900">${bus.price}</p><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Budget Price</p></div>
                  <Button className="rounded-2xl h-14 px-10 font-black text-lg" onClick={() => handleSelectBus(bus)}>Select Seat</Button>
                </div>
              </CardContent>
            </Card>
          ))}
