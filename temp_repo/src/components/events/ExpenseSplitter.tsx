...
import { useUser, useFirestore, updateDocumentNonBlocking, useMemoFirebase, addDocumentNonBlocking } from '@/firebase';
        ...
        <div className="lg:col-span-7 space-y-8">
          <Card className="border-none shadow-xl rounded-[2rem] bg-white overflow-hidden">
            <CardHeader className="bg-slate-900 text-white p-8">
              <CardTitle className="text-xl font-headline font-black uppercase tracking-tighter">Event Ledger</CardTitle>
              <CardDescription className="text-slate-400">Log all group expenditures for this odyssey.</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <div className="divide-y divide-slate-50">
                {expenses.map((exp) => (
                  <div className="p-6 flex items-center gap-4 group hover:bg-slate-50 transition-all" key={exp.id}>
                    <div className="h-10 w-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-primary group-hover:text-white transition-all shadow-inner">
        ...
        <div className="lg:col-span-5 space-y-8">
          {splitResult ? (
              ...
              <Card className="border-none shadow-2xl rounded-[2.5rem] bg-slate-950 text-white p-10 relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-8">
                  ...
                  <div className="space-y-4">
                    <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest border-b border-white/5 pb-2">Optimal Settlement Path</p>
                  ...
                  <p className="text-xs text-slate-400 italic leading-relaxed">"{splitResult.summary}"</p>
                </div>
              </Card>

              <div className="p-6 rounded-3xl bg-primary/5 border border-primary/10 space-y-4">
                <h4 className="text-xs font-black uppercase tracking-widest text-primary flex items-center gap-2">
                  <ShieldCheck className="h-4 w-4" /> Smart Wallet Integration
                </h4>
                <p className="text-xs text-slate-500 font-medium leading-relaxed">
                  Clicking a settlement will generate a dynamic <strong>Scan & Pay</strong> request for the recipient, ensuring high-fidelity financial reconciliation.
                </p>
              </div>
            </div>
          ) : (
            <div className="h-full flex flex-col justify-center text-center opacity-20 grayscale py-20">
              ...
              <p className="text-3xl font-black font-headline uppercase tracking-tighter">Finance Node Pending</p>
              <p className="max-w-xs mx-auto text-sm font-bold mt-2">Log group expenses on the left to synthesize an optimized split strategy.</p>
            </div>
          )}
        </div>
      </div>
