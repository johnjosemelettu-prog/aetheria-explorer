...
export default function VendorLayout({
        ...
        <Card className="w-full max-w-md border-none shadow-2xl rounded-[2.5rem] overflow-hidden">
          ...
          <CardContent className="text-center p-10">
            <p className="text-slate-500 font-medium">
              This portal is reserved for verified Aetheria Partners and Logistics Providers.
            </p>
  ...
  const menuItems = [
    ...
    { label: 'Inventory', href: '/vendor/inventory', icon: Shirt },
    { label: 'Order Hub', href: '/vendor/orders', icon: Package },
    { label: 'Partner Spotlights', href: '/vendor/promotions', icon: Megaphone },
    { label: 'Financials', href: '/vendor/finance', icon: CreditCard },
  ]
    ...
    <SidebarProvider>
      <Sidebar className="border-r-0 shadow-xl">
        <SidebarHeader className="p-6">
          ...
          >
            <div className="relative h-10 w-10 bg-emerald-600 rounded-xl flex items-center justify-center shadow-lg overflow-hidden transition-all group-hover:scale-110">
              <Globe className="absolute h-8 w-8 text-white/10 animate-spin-slow" />
              <Backpack className="h-5 w-5 text-white relative z-10" />
              <div className="absolute -top-0.5 -right-0.5 h-4.5 w-4.5 bg-primary rounded-md flex items-center justify-center border border-emerald-700 shadow-md z-20">
                <Zap className="h-2.5 w-2.5 text-white fill-white" />
              </div>
              <div className="absolute -bottom-0.5 -left-0.5 h-4 w-4 bg-slate-900 rounded-full flex items-center justify-center border border-emerald-700 z-20">
                <Users className="h-2 w-2 text-white" strokeWidth={3} />
              </div>
            </div>
            <div className="flex flex-col items-start leading-none">
              ...
              <span className="text-[6px] font-black uppercase tracking-[0.3em] text-emerald-600">PARTNER HUB</span>
            </div>
          </Link>
        </SidebarHeader>
      ...
      <SidebarInset className="bg-slate-50/50">
        <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b bg-white/80 backdrop-blur-md px-4 sm:px-8">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Aetheria Partner Node</h1>
          </div>
          <UserNav />
        </header>
