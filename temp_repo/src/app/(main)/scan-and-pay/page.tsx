...
export default function ScanAndPayPage() {
  ...
  const handlePay = async () => {
    ...
    try {
      // Simulate High-Fidelity Transaction Verification
      ...
      toast({
        ...
        description: 'Failed to authorize payment. Check your network status.',
      });
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-2xl min-h-screen flex flex-col justify-center">
      <header className="text-center mb-12 space-y-4">
        ...
        <h1 className="font-headline text-4xl font-black tracking-tight md:text-6xl text-slate-900 leading-none italic">
          Scan & Pay
        </h1>
      ...
      {paymentStep === 'scan' && (
        <Card className="overflow-hidden border-none shadow-2xl rounded-[3rem] bg-slate-950">
          <CardHeader className="p-8 pb-4 text-center">
            ...
            <CardDescription className="text-slate-500">Align the visual node within the frame</CardDescription>
          </CardHeader>
            ...
            <div className="relative aspect-square bg-slate-900 flex items-center justify-center">
              {hasCameraPermission === null ? (
                <div className="flex flex-col items-center gap-4 opacity-30">
                  ...
                  <p className="text-xs font-bold text-white uppercase tracking-widest">Waking Sensors...</p>
                </div>
              ) : hasCameraPermission ? (
                <>
                ...
                <div className="text-center p-12 text-white/20 flex flex-col items-center gap-6">
                  ...
                  <p className="font-bold">Camera access restricted. Check browser permissions.</p>
          ...
          <CardFooter className="p-8 bg-slate-900">
            ...
            >
              {isScanning ? <Loader2 className="mr-2 animate-spin h-6 w-6" /> : <Camera className="mr-2 h-6 w-6" />}
      ...
      {paymentStep === 'confirm' && result && (
        <Card className="border-none shadow-2xl rounded-[3rem] overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="bg-slate-900 p-10 text-white relative">
            ...
            <h2 className="text-4xl font-black font-headline leading-tight italic">{result.vendorName}</h2>
            <p className="text-slate-400 mt-2 font-bold uppercase tracking-widest text-xs">Reference: {result.paymentReference}</p>
          </div>
              ...
              {wallets?.find(w => w.currency === result.currency) ? (
                <div className="flex items-center justify-between p-6 rounded-3xl border-2 border-primary bg-primary/5 shadow-inner">
                  <div className="flex items-center gap-4">
                    <div className="h-14 w-14 rounded-2xl bg-primary flex items-center justify-center text-white font-black text-lg">
                      {result.currency}
                    </div>
                    <div>
                  ...
                  <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center text-primary"><ShieldCheck className="h-6 w-6" /></div>
                </div>
              ) : (
                <Alert className="rounded-3xl border-amber-200 bg-amber-50 p-6">
                  ...
                  <AlertDescription className="text-amber-800">You don't have a {result.currency} node initialized. Convert funds in the <Link href="/wallet" className="font-bold underline">Wallet Hub</Link> first.</AlertDescription>
                </Alert>
              )}
            </div>
          </CardContent>
      ...
      {paymentStep === 'success' && result && (
        <Card className="border-none shadow-2xl rounded-[3rem] text-center p-12 space-y-10 animate-in zoom-in duration-700 overflow-hidden relative">
          ...
          <div className="space-y-6">
            ...
            <div>
              <Badge className="bg-emerald-50 text-emerald-700 border-none font-bold uppercase mb-4">Atomic Settlement Verified</Badge>
              <h2 className="text-4xl font-black font-headline text-slate-900 italic tracking-tighter">Transaction Resolved</h2>
