...
export default function StreetFoodRoulettePage() {
  ...
  const spin = () => {
    ...
    const interval = setInterval(() => {
      setSelectedIndex(Math.floor(Math.random() * 5));
      count++;
      if (count > 20) {
    ...
    <div className="container mx-auto px-4 py-12 max-w-4xl">
      <div className="text-center mb-12">
        <h1 className="font-headline text-4xl font-bold tracking-tight md:text-5xl flex items-center justify-center gap-3">
          ...
          {t('foodRoulette.title')}
        </h1>
        <p className="mt-4 text-lg text-muted-foreground">
          {t('foodRoulette.subtitle')}
        </p>
      </div>
        ...
        <form onSubmit={handleFetch} className="flex gap-2">
          <div className="relative flex-grow">
            ...
            <Input 
              ...
              placeholder={t('foodRoulette.inputPlaceholder')} 
              className="pl-10 h-12 rounded-xl"
            />
          </div>
          <Button type="submit" className="h-12 px-8 rounded-xl font-bold" disabled={isLoading}>
            {isLoading ? <Loader2 className="animate-spin" /> : t('foodRoulette.loadBtn')}
          </Button>
        </form>
      </div>
      ...
      {isLoading && (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="font-bold text-muted-foreground animate-pulse">Scouting the best stalls...</p>
        </div>
      )}
    </div>
  );
