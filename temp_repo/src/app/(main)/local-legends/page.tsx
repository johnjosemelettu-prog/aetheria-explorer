...
import { Compass, Sparkles, MapPin, Loader2, Play, Pause, RefreshCw, Wand2, BookOpen } from 'lucide-react';
    ...
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      <div className="text-center mb-12">
        <h1 className="font-headline text-4xl font-bold tracking-tight md:text-5xl flex items-center justify-center gap-3">
          ...
          {t('localLegends.title')}
        </h1>
        <p className="mt-4 text-lg text-muted-foreground">
          {t('localLegends.subtitle')}
        </p>
      </div>
          ...
          <Card className="border-2 border-primary/20 bg-primary/5">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {t('localLegends.proximityTitle')}
              </CardTitle>
              <CardDescription>{t('localLegends.proximityDesc')}</CardDescription>
            </CardHeader>
            <CardContent>
              {isLocating ? (
                ...
                <div className="space-y-4">
                  ...
                  <Button onClick={handleGenerate} className="w-full h-12" disabled={isGenerating}>
                    {isGenerating ? (
                      ...
                      <><Sparkles className="mr-2" /> {t('localLegends.summonBtn')}</>
                    )}
                  </Button>
                </div>
              ) : (
                <p className="text-sm text-destructive">Location access is required for this experience.</p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="overflow-hidden border-4 border-double shadow-2xl bg-zinc-900 aspect-[4/5] relative flex items-center justify-center">
            {isGenerating ? (
              ...
              <div className="text-center p-12 text-zinc-500 space-y-4">
                ...
                <p className="font-headline text-xl">{t('localLegends.radarTitle')}</p>
                <p className="text-sm max-w-xs">{t('localLegends.radarDesc')}</p>
              </div>
            )}
          </Card>
        </div>
